...
pub struct timer_entry {
    ...
    pub prev: *mut timer_entry_t, // Associated Timer Queue
    pub next: *mut timer_entry_t, // Double linked-list
}
...
pub struct timer_queue {
    pub list: *mut timer_entry_t,       // List of timers
...
macro_rules! TIMERQ_LOCK {
    ($queue:expr) => {
        libc::pthread_mutex_lock(addr_of_mut!((*$queue).lock));
    };
}
macro_rules! TIMERQ_UNLOCK {
    ($queue:expr) => {
...
unsafe fn timer_select_queue_from_pool() -> *mut timer_queue_t {
    let mut s_queue: *mut timer_queue_t;
    let mut queue: *mut timer_queue_t;
    let mut level: c_int;
...
unsafe fn timer_schedule_in_queue(queue: *mut timer_queue_t, timer: *mut timer_entry_t) {
    let current: m_tmcnt_t;
    let current_adj: m_tmcnt_t;
...
unsafe fn timer_schedule(timer: *mut timer_entry_t) -> c_int {
    ...
    if queue.is_null() {
        libc::fprintf(c_stderr(), c"timer_schedule: no pool available for timer with ID %llu".as_ptr(), (*timer).id);
        return -1;
    }
...
extern "C" fn timer_loop(queue: *mut c_void) -> *mut c_void {
    unsafe {
        ...
        let mut t_spc: libc::timespec = zeroed();
        let mut timer: *mut timer_entry_t;
        let mut c_time: m_tmcnt_t;
        ...
        loop {
            ...
            if !timer.is_null() {
                ...
                libc::pthread_cond_timedwait(addr_of_mut!((*queue).schedule), addr_of_mut!((*queue).lock), addr_of!(t_spc));
            } else {
...
// Enable a timer
unsafe fn timer_enable(mut timer: *mut timer_entry_t) -> timer_id {
    // Allocate a new ID
    TIMER_LOCK!();
...
pub unsafe extern "C" fn timer_create_with_offset(interval: m_tmcnt_t, _offset: m_tmcnt_t, level: c_int, callback: timer_proc, user_arg: *mut c_void) -> timer_id {
    ...
    (*timer).interval = interval as c_long;
    (*timer).offset = 0; // FIXME offset argument is ignored
    (*timer).callback = callback;
    (*timer).user_arg = user_arg;
    (*timer).flags = 0;
    (*timer).level = level;
...
pub unsafe extern "C" fn timer_flush_queues() {
    let mut timer: *mut timer_entry_t;
    let mut next_timer: *mut timer_entry_t;
    let mut queue: *mut timer_queue_t;
    let mut next_queue: *mut timer_queue_t;
    let mut thread: libc::pthread_t;

    TIMER_LOCK!();

    queue = timer_queue_pool;
    while !queue.is_null() {
        TIMERQ_LOCK!(queue);
...
pub unsafe extern "C" fn timer_init() -> c_int {
    ...
    if timer_id_hash.is_null() {
        libc::fprintf(c_stderr(), c"timer_init: unable to create hash table.".as_ptr());
        return -1;
    }

    // Initialize default queues. If this fails, try to continue.
    if timer_pool_add_queues(TIMERQ_NUMBER) == -1 {
        libc::fprintf(c_stderr(), c"timer_init: unable to initialize at least one timer queue.".as_ptr());
    }
