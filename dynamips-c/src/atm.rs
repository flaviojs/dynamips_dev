...
pub const ATMSW_NIO_MAX: c_int = 32;
...
macro_rules! ATMSW_LOCK {
    ($t:expr) => {
        libc::pthread_mutex_lock(addr_of_mut!((*$t).lock));
    };
}
macro_rules! ATMSW_UNLOCK {
    ($t:expr) => {
...
#[rustfmt::skip]
#[no_mangle]
pub static mut atm_rfc1483b_header: [m_uint8_t; ATM_RFC1483B_HLEN] = [
...
pub unsafe extern "C" fn atm_compute_hec(cell_header: *mut m_uint8_t) -> m_uint8_t {
    ...
    for i in 0..4 {
        hec_accum = hec_syndrome_table[(hec_accum ^ *cell_header.add(i as usize)) as usize];
    }
...
pub unsafe extern "C" fn atmsw_handle_cell(t: *mut atmsw_table_t, input: *mut netio_desc_t, cell: *mut m_uint8_t) -> ssize_t {
    ...
    if !vpc.is_null() {
        ...
        // VC switching
        vcc = atmsw_vc_lookup(t, input, vpi, vci);
        if !vcc.is_null() {
...
pub unsafe extern "C" fn atmsw_create_table(name: *mut c_char) -> *mut atmsw_table_t {
    ...
    mp_create_fixed_pool(addr_of_mut!((*t).mp), c"ATM Switch".as_ptr().cast_mut());
    ...
    if registry_add((*t).name, OBJ_TYPE_ATMSW, t.cast::<_>()) == -1 {
        libc::fprintf(c_stderr(), c"atmsw_create_table: unable to create switch '%s'\n".as_ptr(), name);
        mp_free_pool(addr_of_mut!((*t).mp));
        libc::free(t.cast::<_>());
        return null_mut();
    }
...
pub unsafe extern "C" fn atmsw_create_vcc(t: *mut atmsw_table_t, input: *mut c_char, vpi_in: u_int, vci_in: u_int, output: *mut c_char, vpi_out: u_int, vci_out: u_int) -> c_int {
    ...
    if !atmsw_vp_lookup(t, (*swc).input, vpi_in).is_null() {
        libc::fprintf(c_stderr(), c"atmsw_create_vcc: VP switching already exists for VPI=%u\n".as_ptr(), vpi_in);
        ATMSW_UNLOCK!(t);
...
unsafe extern "C" fn atmsw_free(data: *mut c_void, _arg: *mut c_void) -> c_int {
    ...
    // Remove all VPs
    for i in 0..ATMSW_VP_HASH_SIZE as c_int {
        vp = (*t).vp_table[i as usize];
        while !vp.is_null() {
    ...
    // Remove all VCs
    for i in 0..ATMSW_VC_HASH_SIZE as c_int {
        vc = (*t).vc_table[i as usize];
        while !vc.is_null() {
...
pub unsafe extern "C" fn atmsw_save_config(t: *mut atmsw_table_t, fd: *mut libc::FILE) {
    ...
    libc::fprintf(fd, c"atmsw create %s\n".as_ptr(), (*t).name);
    ...
    for i in 0..ATMSW_VP_HASH_SIZE as c_int {
        vp = (*t).vp_table[i as usize];
        while !vp.is_null() {
            libc::fprintf(fd, c"atmsw create_vpc %s %s %u %s %u\n".as_ptr(), (*t).name, (*(*vp).input).name, (*vp).vpi_in, (*(*vp).output).name, (*vp).vpi_out);
            vp = (*vp).next;
        }
    }
    ...
    for i in 0..ATMSW_VC_HASH_SIZE as c_int {
        vc = (*t).vc_table[i as usize];
        while !vc.is_null() {
            libc::fprintf(fd, c"atmsw create_vcc %s %s %u %u %s %u %u\n".as_ptr(), (*t).name, (*(*vc).input).name, (*vc).vpi_in, (*vc).vci_in, (*(*vc).output).name, (*vc).vpi_out, (*vc).vci_out);
            vc = (*vc).next;
        }
    }
...
unsafe extern "C" fn atmsw_reg_save_config(entry: *mut registry_entry_t, opt: *mut c_void, _err: *mut c_int) {
    atmsw_save_config((*entry).data.cast::<atmsw_table_t>(), opt.cast::<libc::FILE>());
}
...
pub unsafe extern "C" fn atmsw_cfg_create_if(_t: *mut atmsw_table_t, tokens: *mut *mut c_char, count: c_int) -> c_int {
    ...
    if count < 3 {
        libc::fprintf(c_stderr(), c"atmsw_cfg_create_if: invalid interface description\n".as_ptr());
        return -1;
    }
    ...
    match nio_type as u_int {
        NETIO_TYPE_UNIX => 'block: {
            if count != 5 {
                libc::fprintf(c_stderr(), c"ATMSW: invalid number of arguments for UNIX NIO '%s'\n".as_ptr(), *tokens.add(1));
                break 'block;
            }
        ...
        NETIO_TYPE_UDP => 'block: {
            if count != 6 {
                libc::fprintf(c_stderr(), c"ATMSW: invalid number of arguments for UDP NIO '%s'\n".as_ptr(), *tokens.add(1));
                break 'block;
            }
        ...
        NETIO_TYPE_TCP_CLI => 'block: {
            if count != 5 {
                libc::fprintf(c_stderr(), c"ATMSW: invalid number of arguments for TCP CLI NIO '%s'\n".as_ptr(), *tokens.add(1));
                break 'block;
            }
        ...
        NETIO_TYPE_TCP_SER => 'block: {
            if count != 4 {
                libc::fprintf(c_stderr(), c"ATMSW: invalid number of arguments for TCP SER NIO '%s'\n".as_ptr(), *tokens.add(1));
                break 'block;
            }
        ...
        _ => {
            libc::fprintf(c_stderr(), c"ATMSW: unknown/invalid NETIO type '%s'\n".as_ptr(), *tokens.add(2));
        }
    }

    if nio.is_null() {
        libc::fprintf(c_stderr(), c"ATMSW: unable to create NETIO descriptor of interface %s\n".as_ptr(), *tokens.add(1));
        return -1;
    }
...
pub unsafe extern "C" fn atmsw_cfg_create_vpc(t: *mut atmsw_table_t, tokens: *mut *mut c_char, count: c_int) -> c_int {
    ...
    if count != 5 {
        libc::fprintf(c_stderr(), c"ATMSW: invalid VPC descriptor.\n".as_ptr());
        return -1;
    }
...
pub unsafe extern "C" fn atmsw_cfg_create_vcc(t: *mut atmsw_table_t, tokens: *mut *mut c_char, count: c_int) -> c_int {
    ...
    if count != 7 {
        libc::fprintf(c_stderr(), c"ATMSW: invalid VCC descriptor.\n".as_ptr());
        return -1;
    }
    ...
    atmsw_create_vcc(
        t,
        *tokens.add(1),
        libc::atoi(*tokens.add(2)) as u_int,
        libc::atoi(*tokens.add(3)) as u_int,
        *tokens.add(4),
        libc::atoi(*tokens.add(5)) as u_int,
        libc::atoi(*tokens.add(6)) as u_int,
    )
}
...
pub unsafe extern "C" fn atmsw_handle_cfg_line(t: *mut atmsw_table_t, str_: *mut c_char) -> c_int {
    ...
    let count: c_int = m_strsplit(str_, b':' as c_char, tokens.as_mut_ptr(), ATMSW_MAX_TOKENS as c_int);
    if count <= 1 {
        return -1;
    }

    if 0 == libc::strcmp(tokens[0], c"IF".as_ptr()) {
        return atmsw_cfg_create_if(t, tokens.as_mut_ptr(), count);
    } else if 0 == libc::strcmp(tokens[0], c"VP".as_ptr()) {
        return atmsw_cfg_create_vpc(t, tokens.as_mut_ptr(), count);
    } else if 0 == libc::strcmp(tokens[0], c"VC".as_ptr()) {
        return atmsw_cfg_create_vcc(t, tokens.as_mut_ptr(), count);
    }

    libc::fprintf(c_stderr(), c"ATMSW: Unknown statement \"%s\" (allowed: IF,VP,VC)\n".as_ptr(), tokens[0]);
    -1
}
...
pub unsafe extern "C" fn atmsw_read_cfg_file(t: *mut atmsw_table_t, filename: *mut c_char) -> c_int {
    ...
    while 0 == libc::feof(fd) {
        ...
        if !libc::strchr(buffer.as_ptr(), b':' as c_int).is_null() {
            atmsw_handle_cfg_line(t, buffer.as_mut_ptr());
        }
    }
...
pub unsafe extern "C" fn atmsw_start(filename: *mut c_char) -> c_int {
    let t: *mut atmsw_table_t = atmsw_create_table(c"default".as_ptr().cast_mut());
    if t.is_null() {
        libc::fprintf(c_stderr(), c"ATMSW: unable to create virtual fabric table.\n".as_ptr());
        return -1;
    }

    if atmsw_read_cfg_file(t, filename) == -1 {
        libc::fprintf(c_stderr(), c"ATMSW: unable to parse configuration file.\n".as_ptr());
        return -1;
    }

    atmsw_release(c"default".as_ptr().cast_mut());
    0
}
