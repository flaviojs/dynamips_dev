...
// EEPROM types
pub type _EEPROM_TYPE_ENUM = u_int; // TODO enum
pub const EEPROM_TYPE_NMC93C46: _EEPROM_TYPE_ENUM = 0;
pub const EEPROM_TYPE_NMC93C56: _EEPROM_TYPE_ENUM = 1;

// EEPROM data bit order
pub type _EEPROM_DORD_ENUM = c_int; // TODO enum
pub const EEPROM_DORD_NORMAL: _EEPROM_DORD_ENUM = 0;
pub const EEPROM_DORD_REVERSED: _EEPROM_DORD_ENUM = 1;

// EEPROM debugging
pub type _EEPROM_DEBUG_ENUM = c_int; // TODO enum
pub const EEPROM_DEBUG_DISABLED: _EEPROM_DEBUG_ENUM = 0;
pub const EEPROM_DEBUG_ENABLED: _EEPROM_DEBUG_ENUM = 1;

// EEPROM DOUT default status
pub type _EEPROM_DOUT_ENUM = u_int; // TODO enum
pub const EEPROM_DOUT_HIGH: _EEPROM_DOUT_ENUM = 0;
pub const EEPROM_DOUT_KEEP: _EEPROM_DOUT_ENUM = 1;
...
// Internal states
type _EEPROM_STATE_ENUM = u_int; // TODO enum
const EEPROM_STATE_INACTIVE: _EEPROM_STATE_ENUM = 0;
const EEPROM_STATE_WAIT_CMD: _EEPROM_STATE_ENUM = 1;
const EEPROM_STATE_DATAOUT: _EEPROM_STATE_ENUM = 2;
...
unsafe fn nmc93cX6_check_cs(g: *mut nmc93cX6_group, old: u_int, new: u_int) {
    ...
    for i in 0..(*g).nr_eeprom as c_int {
        ...
        if (*g).debug != 0 {
            libc::printf(
                c"EEPROM %s(%d): check_cs:  check_bit(old,new,select_bit) [%8.8x, %8.8x, %d (mask = %8.8x)] = %d\n".as_ptr(),
                (*g).description,
                i,
                old,
                new,
                (*(*g).def[i as usize]).select_bit,
                1 << (*(*g).def[i as usize]).select_bit,
                check_bit(old, new, (*(*g).def[i as usize]).select_bit),
            );
        }

        res = check_bit(old, new, (*(*g).def[i as usize]).select_bit);
        if res != 0 {
...
unsafe fn nmc93cX6_check_clk_group(g: *mut nmc93cX6_group, group_id: c_int, old: u_int, new: u_int) {
    ...
    if (*g).debug != 0 {
        libc::printf(
            c"EEPROM %s(%d): check_clk: check_bit(old,new,select_bit) [%8.8x, %8.8x, %d (mask = %8.8x)] = %d\n".as_ptr(),
            (*g).description,
            group_id,
            old,
            new,
    ...
    match (*g).state[group_id as usize].state {
        EEPROM_STATE_WAIT_CMD => 'block: {
            ...
            if ((*g).state[group_id as usize].cmd_len == 0) && 0 == (new & (1 << din_bit)) {
                break 'block;
            }
            ...
            if (*g).state[group_id as usize].cmd_len == cmd_len {
                if DEBUG_EEPROM != 0 {
                    libc::printf(c"nmc93cX6: %s(%d): command = %x\n".as_ptr(), (*g).description, group_id, (*g).state[group_id as usize].cmd_val);
                }
                ...
                match op {
                    ...
                    _ => {
                        if DEBUG_EEPROM != 0 {
                            libc::printf(c"nmc93cX6: unhandled opcode %d\n".as_ptr(), op);
                        }
                    }
                }
            }
        ...
        EEPROM_STATE_DATAOUT => {
            ...
            if DEBUG_EEPROM != 0 {
                if (*g).state[group_id as usize].dataout_pos == 0 {
                    libc::printf(
                        c"nmc93cX6: %s(%d): read addr=%x (%d), val=%4.4x [eeprom=%p]\n".as_ptr(),
                        (*g).description,
                        group_id,
                        addr,
                        addr,
                        (*g).state[group_id as usize].cmd_val,
                        (*g).eeprom[group_id as usize],
                    );
                }
            }
        ...
        _ => {
            if DEBUG_EEPROM != 0 {
                libc::printf(c"nmc93cX6: unhandled state %d\n".as_ptr(), (*g).state[group_id as usize].state);
            }
        }
    }
}
...
pub unsafe extern "C" fn nmc93cX6_read(g: *mut nmc93cX6_group) -> u_int {
    ...
    for i in 0..(*g).nr_eeprom as c_int {
        if 0 == ((*g).eeprom_reg & (1 << (*(*g).def[i as usize]).select_bit)) {
            continue;
        }
