...
pub type _NETIO_TYPE_ENUM = u_int; // TODO enum
pub const NETIO_TYPE_UNIX: _NETIO_TYPE_ENUM = 0;
pub const NETIO_TYPE_VDE: _NETIO_TYPE_ENUM = 1;
pub const NETIO_TYPE_TAP: _NETIO_TYPE_ENUM = 2;
pub const NETIO_TYPE_UDP: _NETIO_TYPE_ENUM = 3;
pub const NETIO_TYPE_UDP_AUTO: _NETIO_TYPE_ENUM = 4;
pub const NETIO_TYPE_TCP_CLI: _NETIO_TYPE_ENUM = 5;
pub const NETIO_TYPE_TCP_SER: _NETIO_TYPE_ENUM = 6;
#[cfg(all(feature = "ENABLE_LINUX_ETH", feature = "ENABLE_GEN_ETH"))]
pub const NETIO_TYPE_LINUX_ETH: _NETIO_TYPE_ENUM = 7;
#[cfg(all(feature = "ENABLE_LINUX_ETH", feature = "ENABLE_GEN_ETH"))]
pub const NETIO_TYPE_GEN_ETH: _NETIO_TYPE_ENUM = 8;
#[cfg(all(feature = "ENABLE_LINUX_ETH", feature = "ENABLE_GEN_ETH"))]
pub const NETIO_TYPE_FIFO: _NETIO_TYPE_ENUM = 9;
#[cfg(all(feature = "ENABLE_LINUX_ETH", feature = "ENABLE_GEN_ETH"))]
pub const NETIO_TYPE_NULL: _NETIO_TYPE_ENUM = 10;
#[cfg(all(feature = "ENABLE_LINUX_ETH", feature = "ENABLE_GEN_ETH"))]
pub const NETIO_TYPE_MAX: _NETIO_TYPE_ENUM = 11;
#[cfg(all(not(feature = "ENABLE_LINUX_ETH"), feature = "ENABLE_GEN_ETH"))]
pub const NETIO_TYPE_GEN_ETH: _NETIO_TYPE_ENUM = 7;
#[cfg(all(not(feature = "ENABLE_LINUX_ETH"), feature = "ENABLE_GEN_ETH"))]
pub const NETIO_TYPE_FIFO: _NETIO_TYPE_ENUM = 8;
#[cfg(all(not(feature = "ENABLE_LINUX_ETH"), feature = "ENABLE_GEN_ETH"))]
pub const NETIO_TYPE_NULL: _NETIO_TYPE_ENUM = 9;
#[cfg(all(not(feature = "ENABLE_LINUX_ETH"), feature = "ENABLE_GEN_ETH"))]
pub const NETIO_TYPE_MAX: _NETIO_TYPE_ENUM = 10;
...
pub const NETIO_TYPE_FIFO: _NETIO_TYPE_ENUM = 7;
#[cfg(all(not(feature = "ENABLE_LINUX_ETH"), not(feature = "ENABLE_GEN_ETH")))]
pub const NETIO_TYPE_NULL: _NETIO_TYPE_ENUM = 8;
#[cfg(all(not(feature = "ENABLE_LINUX_ETH"), not(feature = "ENABLE_GEN_ETH")))]
pub const NETIO_TYPE_MAX: _NETIO_TYPE_ENUM = 9;

pub type _NETIO_FILTER_ACTION_ENUM = c_int; // TODO enum
pub const NETIO_FILTER_ACTION_DROP: _NETIO_FILTER_ACTION_ENUM = 0;
pub const NETIO_FILTER_ACTION_PASS: _NETIO_FILTER_ACTION_ENUM = 1;
pub const NETIO_FILTER_ACTION_ALTER: _NETIO_FILTER_ACTION_ENUM = 2;
pub const NETIO_FILTER_ACTION_DUPLICATE: _NETIO_FILTER_ACTION_ENUM = 3;
...
pub enum vde_request_type {
    VDE_REQ_NEW_CONTROL = 0,
}

pub const VDE_SWITCH_MAGIC: u32 = 0xfeedface;
pub const VDE_SWITCH_VERSION: u32 = 3;
...
pub struct netio_desc {
    ...
    pub rx_filter_data: *mut ::std::os::raw::c_void,
    pub tx_filter_data: *mut ::std::os::raw::c_void,
    pub both_filter_data: *mut ::std::os::raw::c_void,
...
static mut netio_rxl_thread: libc::pthread_t = unsafe { zeroed() };
static mut netio_rxl_cond: libc::pthread_cond_t = libc::PTHREAD_COND_INITIALIZER;

macro_rules! NETIO_RXL_LOCK {
    () => {
        libc::pthread_mutex_lock(addr_of_mut!(netio_rxl_mutex));
    };
}
macro_rules! NETIO_RXL_UNLOCK {
    () => {
        libc::pthread_mutex_unlock(addr_of_mut!(netio_rxl_mutex));
    };
}

macro_rules! NETIO_RXQ_LOCK {
    () => {
        libc::pthread_mutex_lock(addr_of_mut!(netio_rxq_mutex));
    };
}
macro_rules! NETIO_RXQ_UNLOCK {
    () => {
...
#[repr(C)]
#[derive(Debug, Copy, Clone)]
pub struct netio_type_t {
    pub name: *mut c_char,
    pub desc: *mut c_char,
}

// NETIO types (must follow the enum definition)
static mut netio_types: [netio_type_t; NETIO_TYPE_MAX as usize] = [
    netio_type_t { name: c"unix".as_ptr().cast_mut(), desc: c"UNIX local sockets".as_ptr().cast_mut() },
    netio_type_t { name: c"vde".as_ptr().cast_mut(), desc: c"Virtual Distributed Ethernet / UML switch".as_ptr().cast_mut() },
    netio_type_t { name: c"tap".as_ptr().cast_mut(), desc: c"Linux/FreeBSD TAP device".as_ptr().cast_mut() },
    netio_type_t { name: c"udp".as_ptr().cast_mut(), desc: c"UDP sockets".as_ptr().cast_mut() },
    netio_type_t { name: c"udp_auto".as_ptr().cast_mut(), desc: c"Auto UDP sockets".as_ptr().cast_mut() },
    netio_type_t { name: c"tcp_cli".as_ptr().cast_mut(), desc: c"TCP client".as_ptr().cast_mut() },
    netio_type_t { name: c"tcp_ser".as_ptr().cast_mut(), desc: c"TCP server".as_ptr().cast_mut() },
    #[cfg(feature = "ENABLE_LINUX_ETH")]
    netio_type_t { name: c"linux_eth".as_ptr().cast_mut(), desc: c"Linux Ethernet device".as_ptr().cast_mut() },
    #[cfg(feature = "ENABLE_GEN_ETH")]
    netio_type_t { name: c"gen_eth".as_ptr().cast_mut(), desc: c"Generic Ethernet device (PCAP)".as_ptr().cast_mut() },
    netio_type_t { name: c"fifo".as_ptr().cast_mut(), desc: c"FIFO (intra-hypervisor)".as_ptr().cast_mut() },
    netio_type_t { name: c"null".as_ptr().cast_mut(), desc: c"Null device".as_ptr().cast_mut() },
];

// Get NETIO type given a description
#[no_mangle]
pub unsafe extern "C" fn netio_get_type(type_: *mut c_char) -> c_int {
    for i in 0..NETIO_TYPE_MAX as c_int {
        if 0 == libc::strcmp(type_, netio_types[i as usize].name) {
            return i;
        }
    }
...
pub unsafe extern "C" fn netio_show_types() {
    libc::printf(c"Available NETIO types:\n".as_ptr());

    for i in 0..NETIO_TYPE_MAX {
        libc::printf(c"  * %-10s : %s\n".as_ptr(), netio_types[i as usize].name, netio_types[i as usize].desc);
    }
...
pub unsafe extern "C" fn netio_save_config(nio: *mut netio_desc_t, fd: *mut libc::FILE) {
    if let Some(save_cfg) = (*nio).save_cfg {
        save_cfg(nio, fd);
    }
}
...
pub unsafe extern "C" fn netio_send(nio: *mut netio_desc_t, pkt: *mut c_void, len: size_t) -> ssize_t {
    ...
    if (*nio).debug != 0 {
        libc::printf(c"NIO %s: sending a packet of %lu bytes:\n".as_ptr(), (*nio).name, len as u_long);
        mem_dump(c_stdout(), pkt.cast::<_>(), len as u_int);
    }

    // Apply the TX filter
    if !(*nio).tx_filter.is_null() {
        res = (*(*nio).tx_filter).pkt_handler.unwrap()(nio, pkt, len, (*nio).tx_filter_data);
    ...
    if !(*nio).both_filter.is_null() {
        res = (*(*nio).both_filter).pkt_handler.unwrap()(nio, pkt, len, (*nio).both_filter_data);
...
pub unsafe extern "C" fn netio_recv(nio: *mut netio_desc_t, pkt: *mut c_void, max_len: size_t) -> ssize_t {
    ...
    // Receive the packet
    libc::memset(pkt.cast::<_>(), 0, max_len);
    ...
    if (*nio).debug != 0 {
        libc::printf(c"NIO %s: receiving a packet of %ld bytes:\n".as_ptr(), (*nio).name, len as c_long);
        mem_dump(c_stdout(), pkt.cast::<_>(), len as u_int);
    }

    // Apply the RX filter
    if !(*nio).rx_filter.is_null() {
        res = (*(*nio).rx_filter).pkt_handler.unwrap()(nio, pkt, len as size_t, (*nio).rx_filter_data);
    ...
    // Apply the bidirectional filter
    if !(*nio).both_filter.is_null() {
        res = (*(*nio).both_filter).pkt_handler.unwrap()(nio, pkt, len as size_t, (*nio).both_filter_data);
...
unsafe fn netio_unix_create_socket(nud: *mut netio_unix_desc_t) -> c_int {
    let mut local_sock: libc::sockaddr_un = zeroed();

    (*nud).fd = libc::socket(libc::AF_UNIX, libc::SOCK_DGRAM, 0);
    if (*nud).fd == -1 {
        libc::perror(c"netio_unix: socket".as_ptr());
        return -1;
    }

    libc::memset(addr_of_mut!(local_sock).cast::<_>(), 0, size_of::<libc::sockaddr_un>());
    local_sock.sun_family = libc::AF_UNIX as _;
    libc::strncpy(local_sock.sun_path.as_mut_ptr(), (*nud).local_filename, local_sock.sun_path.len() - 1);
    local_sock.sun_path[local_sock.sun_path.len() - 1] = b'\0' as c_char;

    if libc::bind((*nud).fd, addr_of!(local_sock).cast::<_>(), size_of::<libc::sockaddr_un>() as _) == -1 {
        libc::perror(c"netio_unix: bind".as_ptr());
        return -1;
    }
...
unsafe fn netio_unix_create(nud: *mut netio_unix_desc_t, local: *mut c_char, remote: *mut c_char) -> c_int {
    ...
    if (libc::strlen(local) >= (*nud).remote_sock.sun_path.len()) || (libc::strlen(remote) >= (*nud).remote_sock.sun_path.len()) {
        libc::fprintf(c_stderr(), c"netio_unix_create: invalid file size or insufficient memory\n".as_ptr());
        return -1;
    }
    ...
    (*nud).remote_sock.sun_family = libc::AF_UNIX as _;
    libc::strcpy((*nud).remote_sock.sun_path.as_mut_ptr(), remote);
    0
}
...
unsafe extern "C" fn netio_unix_send(nud: *mut c_void, pkt: *mut c_void, pkt_len: size_t) -> ssize_t {
    let nud: *mut netio_unix_desc_t = nud.cast::<_>();
    libc::sendto((*nud).fd, pkt, pkt_len, 0, addr_of!((*nud).remote_sock).cast::<_>(), size_of::<libc::sockaddr_un>() as _)
}
...
unsafe extern "C" fn netio_unix_save_cfg(nio: *mut netio_desc_t, fd: *mut libc::FILE) {
    ...
    libc::fprintf(fd, c"nio create_unix %s %s %s\n".as_ptr(), (*nio).name, (*nud).local_filename, (*nud).remote_sock.sun_path);
}
...
unsafe fn netio_vde_create(nvd: *mut netio_vde_desc_t, control: *mut c_char, local: *mut c_char) -> c_int {
    let mut ctrl_sock: libc::sockaddr_un = zeroed();
    let mut tst: libc::sockaddr_un = zeroed();
    let mut req: vde_request_v3 = zeroed();
    let mut len: ssize_t;
    ...
    if (libc::strlen(control) >= ctrl_sock.sun_path.len()) || (libc::strlen(local) >= (*nvd).remote_sock.sun_path.len()) {
        libc::fprintf(c_stderr(), c"netio_vde_create: bad filenames specified\n".as_ptr());
        return -1;
    }
    ...
    if (*nvd).local_filename.is_null() {
        libc::fprintf(c_stderr(), c"netio_vde_create: insufficient memory\n".as_ptr());
        return -1;
    }
    ...
    if (*nvd).ctrl_fd < 0 {
        libc::perror(c"netio_vde_create: socket(control)".as_ptr());
        return -1;
    }

    libc::memset(addr_of_mut!(ctrl_sock).cast::<_>(), 0, size_of::<libc::sockaddr_un>());
    ctrl_sock.sun_family = libc::AF_UNIX as _;
    libc::strcpy(ctrl_sock.sun_path.as_mut_ptr(), control);

    let res: c_int = libc::connect((*nvd).ctrl_fd, addr_of!(ctrl_sock).cast::<_>(), size_of::<libc::sockaddr_un>() as _);

    if res < 0 {
        libc::perror(c"netio_vde_create: connect(control)".as_ptr());
        return -1;
    }

    tst.sun_family = libc::AF_UNIX as _;
    libc::strcpy(tst.sun_path.as_mut_ptr(), local);

    // Create the data connection
    (*nvd).data_fd = libc::socket(libc::AF_UNIX, libc::SOCK_DGRAM, 0);
    if (*nvd).data_fd < 0 {
        libc::perror(c"netio_vde_create: socket(data)".as_ptr());
        return -1;
    }

    if libc::bind((*nvd).data_fd, addr_of!(tst).cast::<_>(), size_of::<libc::sockaddr_un>() as _) < 0 {
        libc::perror(c"netio_vde_create: bind(data)".as_ptr());
        return -1;
    }

    // Now, process to registration
    libc::memset(addr_of_mut!(req).cast::<_>(), 0, size_of::<vde_request_v3>());
    req.sock.sun_family = libc::AF_UNIX as _;
    libc::strcpy(req.sock.sun_path.as_mut_ptr(), local);
    ...
    if len != size_of::<vde_request_v3>() as ssize_t {
        libc::perror(c"netio_vde_create: write(req)".as_ptr());
        return -1;
    }
    ...
    if len != size_of::<libc::sockaddr_un>() as ssize_t {
        libc::perror(c"netio_vde_create: read(req)".as_ptr());
        return -1;
    }
...
unsafe extern "C" fn netio_vde_send(nvd: *mut c_void, pkt: *mut c_void, pkt_len: size_t) -> ssize_t {
    let nvd: *mut netio_vde_desc_t = nvd.cast::<_>();
    libc::sendto((*nvd).data_fd, pkt, pkt_len, 0, addr_of!((*nvd).remote_sock).cast::<_>(), size_of::<libc::sockaddr_un>() as _)
}
...
unsafe extern "C" fn netio_vde_save_cfg(nio: *mut netio_desc_t, fd: *mut libc::FILE) {
    ...
    libc::fprintf(fd, c"nio create_vde %s %s %s\n".as_ptr(), (*nio).name, (*nvd).remote_sock.sun_path.as_ptr(), (*nvd).local_filename);
}
...
unsafe fn netio_tap_open(tap_devname: *mut c_char) -> c_int {
    #[cfg(target_os = "linux")]
    {
        ...
        let fd: c_int = libc::open(c"/dev/net/tun".as_ptr(), libc::O_RDWR);
        if fd < 0 {
            return -1;
        }
        ...
        ifr.ifr_ifru.ifru_flags = (libc::IFF_TAP | libc::IFF_NO_PI) as _;
        if *tap_devname != 0 {
            libc::strncpy(ifr.ifr_name.as_mut_ptr(), tap_devname, libc::IFNAMSIZ - 1);
            ifr.ifr_name[libc::IFNAMSIZ - 1] = b'\0' as c_char;
        }

        let err: c_int = libc::ioctl(fd, libc::TUNSETIFF, addr_of!(ifr).cast::<c_void>());
        if err < 0 {
            libc::close(fd);
            return err;
        }

        libc::strcpy(tap_devname, ifr.ifr_name.as_ptr());
        fd
    }
    #[cfg(not(target_os = "linux"))]
    {
        ...
        if *tap_devname != 0 {
            libc::snprintf(tap_fullname.as_mut_ptr(), NETIO_DEV_MAXLEN, c"/dev/%s".as_ptr(), tap_devname);
            fd = libc::open(tap_fullname.as_ptr(), libc::O_RDWR);
        } else {
            for i in 0..16 as c_int {
                libc::snprintf(tap_devname, NETIO_DEV_MAXLEN, c"/dev/tap%d".as_ptr(), i);
...
unsafe fn netio_tap_create(ntd: *mut netio_tap_desc_t, tap_name: *mut c_char) -> c_int {
    if libc::strlen(tap_name) >= NETIO_DEV_MAXLEN {
        libc::fprintf(c_stderr(), c"netio_tap_create: bad TAP device string specified.\n".as_ptr());
        return -1;
    }

    libc::memset(ntd.cast::<_>(), 0, size_of::<netio_tap_desc_t>());
    libc::strcpy((*ntd).filename.as_mut_ptr(), tap_name);
    (*ntd).fd = netio_tap_open((*ntd).filename.as_mut_ptr());

    if (*ntd).fd == -1 {
        libc::fprintf(c_stderr(), c"netio_tap_create: unable to open TAP device %s (%s)\n".as_ptr(), tap_name, libc::strerror(c_errno()));
        return -1;
    }
...
unsafe extern "C" fn netio_tap_save_cfg(nio: *mut netio_desc_t, fd: *mut libc::FILE) {
    ...
    libc::fprintf(fd, c"nio create_tap %s %s\n".as_ptr(), (*nio).name, (*ntd).filename);
}
...
unsafe extern "C" fn netio_tcp_send(nid: *mut c_void, pkt: *mut c_void, pkt_len: size_t) -> ssize_t {
    let nid: *mut netio_inet_desc_t = nid.cast::<_>();
    let l: m_uint32_t = libc::htonl(pkt_len as m_uint32_t);
...
unsafe fn netio_tcp_cli_create(nid: *mut netio_inet_desc_t, host: *mut c_char, port: *mut c_char) -> c_int {
    ...
    if libc::atoi(port) == 0 {
        sp = libc::getservbyname(port, c"tcp".as_ptr());
        if sp.is_null() {
            libc::fprintf(c_stderr(), c"netio_tcp_cli_create: port %s is neither number not service %s\n".as_ptr(), port, libc::strerror(c_errno()));
            libc::close((*nid).fd);
            return -1;
        }
        serv.sin_port = (*sp).s_port as _;
    } else {
        serv.sin_port = libc::htons(libc::atoi(port) as _);
    }

    if _sys::inet_addr(host) == libc::INADDR_NONE {
        hp = _sys::gethostbyname(host) as _;
        if hp.is_null() {
            libc::fprintf(c_stderr(), c"netio_tcp_cli_create: no host %s\n".as_ptr(), host);
            libc::close((*nid).fd);
            return -1;
        }
        serv.sin_addr.s_addr = (*(*hp).h_addr_list) as usize as _;
    } else {
        serv.sin_addr.s_addr = _sys::inet_addr(host);
    }

    if libc::connect((*nid).fd, addr_of!(serv).cast::<_>(), size_of::<libc::sockaddr_in>() as _) < 0 {
        libc::fprintf(c_stderr(), c"netio_tcp_cli_create: connect to %s:%s failed %s\n".as_ptr(), host, port, libc::strerror(c_errno()));
        libc::close((*nid).fd);
        return -1;
    }
    0
...
#[no_mangle]
pub unsafe extern "C" fn netio_tcp_ser_create(nid: *mut netio_inet_desc_t, port: *mut c_char) -> c_int {
    let mut serv: libc::sockaddr_in = zeroed();
    let sp: *mut libc::servent;

    let sock_fd: c_int = libc::socket(libc::PF_INET, libc::SOCK_STREAM, 0);
    if sock_fd < 0 {
        libc::perror(c"netio_tcp_cli_create: socket\n".as_ptr());
        return -1;
    }

    libc::memset(addr_of_mut!(serv).cast::<_>(), 0, size_of::<libc::sockaddr_in>());
    serv.sin_family = libc::AF_INET as _;
    serv.sin_addr.s_addr = libc::htonl(libc::INADDR_ANY);

    if libc::atoi(port) == 0 {
        sp = libc::getservbyname(port, c"tcp".as_ptr());
        if sp.is_null() {
            libc::fprintf(c_stderr(), c"netio_tcp_ser_create: port %s is neither number not service %s\n".as_ptr(), port, libc::strerror(c_errno()));
            libc::close(sock_fd);
            return -1;
        }
        serv.sin_port = (*sp).s_port as _;
    ...
    if libc::bind(sock_fd, addr_of!(serv).cast::<_>(), size_of::<libc::sockaddr_in>() as _) < 0 {
        libc::fprintf(c_stderr(), c"netio_tcp_ser_create: bind %s failed %s\n".as_ptr(), port, libc::strerror(c_errno()));
        libc::close(sock_fd);
        return -1;
    }

    if libc::listen(sock_fd, 1) < 0 {
        libc::fprintf(c_stderr(), c"netio_tcp_ser_create: listen %s failed %s\n".as_ptr(), port, libc::strerror(c_errno()));
        libc::close(sock_fd);
        return -1;
    }

    libc::fprintf(c_stderr(), c"Waiting connection on port %s...\n".as_ptr(), port);

    (*nid).fd = libc::accept(sock_fd, null_mut(), null_mut());
    if (*nid).fd < 0 {
        libc::fprintf(c_stderr(), c"netio_tcp_ser_create: accept %s failed %s\n".as_ptr(), port, libc::strerror(c_errno()));
        libc::close(sock_fd);
        return -1;
    }

    libc::fprintf(c_stderr(), c"Connected\n".as_ptr());
...
unsafe extern "C" fn netio_udp_save_cfg(nio: *mut netio_desc_t, fd: *mut libc::FILE) {
    ...
    libc::fprintf(fd, c"nio create_udp %s %d %s %d\n".as_ptr(), (*nio).name, (*nid).local_port, (*nid).remote_host, (*nid).remote_port);
}
...
pub unsafe extern "C" fn netio_desc_create_udp(nio_name: *mut c_char, local_port: c_int, remote_host: *mut c_char, remote_port: c_int) -> *mut netio_desc_t {
    ...
    if (*nid).remote_host.is_null() {
        libc::fprintf(c_stderr(), c"netio_desc_create_udp: insufficient memory\n".as_ptr());
        netio_free(nio.cast::<_>(), null_mut());
        return null_mut();
    }
    ...
    if (*nid).fd < 0 {
        libc::fprintf(c_stderr(), c"netio_desc_create_udp: unable to connect to %s:%d\n".as_ptr(), remote_host, remote_port);
        netio_free(nio.cast::<_>(), null_mut());
        return null_mut();
    }
...
pub unsafe extern "C" fn netio_udp_auto_connect(nio: *mut netio_desc_t, host: *mut c_char, port: c_int) -> c_int {
    ...
    if (*nid).remote_host.is_null() {
        libc::fprintf(c_stderr(), c"netio_desc_create_udp_auto: insufficient memory\n".as_ptr());
        return -1;
    }
...
pub unsafe extern "C" fn netio_desc_create_udp_auto(nio_name: *mut c_char, local_addr: *mut c_char, port_start: c_int, port_end: c_int) -> *mut netio_desc_t {
    ...
    if (*nid).fd < 0 {
        libc::fprintf(c_stderr(), c"netio_desc_create_udp_auto: unable to create socket (addr=%s,port_start=%d,port_end=%d)\n".as_ptr(), local_addr, port_start, port_end);
        netio_free(nio.cast::<_>(), null_mut());
        return null_mut();
    }
...
unsafe extern "C" fn netio_lnxeth_save_cfg(nio: *mut netio_desc_t, fd: *mut libc::FILE) {
    ...
    libc::fprintf(fd, c"nio create_linux_eth %s %s\n".as_ptr(), (*nio).name, (*nled).dev_name);
}
...
pub unsafe extern "C" fn netio_desc_create_lnxeth(nio_name: *mut c_char, dev_name: *mut c_char) -> *mut netio_desc_t {
    ...
    if libc::strlen(dev_name) >= NETIO_DEV_MAXLEN {
        libc::fprintf(c_stderr(), c"netio_desc_create_lnxeth: bad Ethernet device string specified.\n".as_ptr());
        netio_free(nio.cast::<_>(), null_mut());
        return null_mut();
    }

    libc::strcpy((*nled).dev_name.as_mut_ptr(), dev_name);
...
unsafe extern "C" fn netio_geneth_save_cfg(nio: *mut netio_desc_t, fd: *mut libc::FILE) {
    ...
    libc::fprintf(fd, c"nio create_gen_eth %s %s\n".as_ptr(), (*nio).name, (*nged).dev_name);
}
...
pub unsafe extern "C" fn netio_desc_create_geneth(nio_name: *mut c_char, dev_name: *mut c_char) -> *mut netio_desc_t {
    ...
    if libc::strlen(dev_name) >= NETIO_DEV_MAXLEN {
        libc::fprintf(c_stderr(), c"netio_desc_create_geneth: bad Ethernet device string specified.\n".as_ptr());
        netio_free(nio.cast::<_>(), null_mut());
        return null_mut();
    }

    libc::strcpy((*nged).dev_name.as_mut_ptr(), dev_name);
...
unsafe extern "C" fn netio_fifo_send(nfd: *mut c_void, pkt: *mut c_void, pkt_len: size_t) -> ssize_t {
    ...
    libc::memcpy((*p).pkt.as_mut_ptr().cast::<_>(), pkt, pkt_len);
    (*p).pkt_len = pkt_len;
...
unsafe extern "C" fn netio_fifo_recv(nfd: *mut c_void, pkt: *mut c_void, max_len: size_t) -> ssize_t {
    ...
    if !p.is_null() {
        ...
        libc::memcpy(pkt, (*p).pkt.as_ptr().cast::<_>(), len);
        libc::free(p.cast::<_>());
    }
...
unsafe extern "C" fn netio_null_save_cfg(nio: *mut netio_desc_t, fd: *mut libc::FILE) {
    libc::fprintf(fd, c"nio create_null %s\n".as_ptr(), (*nio).name);
}
...
pub unsafe extern "C" fn netio_can_transmit(nio: *mut netio_desc_t) -> c_int {
    ...
    (bw_current < (*nio).bandwidth) as c_int
}
...
extern "C" fn netio_rxl_spec_thread(arg: *mut c_void) -> *mut c_void {
    unsafe {
        ...
        while (*rxl).running.get() != 0 {
            pkt_len = netio_recv(nio, (*nio).rx_pkt.as_mut_ptr().cast::<_>(), (*nio).rx_pkt.len());

            if pkt_len > 0 {
                (*rxl).rx_handler.unwrap()(nio, (*nio).rx_pkt.as_mut_ptr(), pkt_len, (*rxl).arg1, (*rxl).arg2);
            }
        }
...
extern "C" fn netio_rxl_gen_thread(_arg: *mut c_void) -> *mut c_void {
    unsafe {
        ...
        let mut res: c_int;
        let mut rfds: libc::fd_set = zeroed();
        ...
        loop {
            ...
            if res == -1 {
                if c_errno() != libc::EINTR {
                    libc::perror(c"netio_rxl_thread: select".as_ptr());
                }
                continue;
            }
...
pub unsafe extern "C" fn netio_rxl_add(nio: *mut netio_desc_t, rx_handler: netio_rx_handler_t, arg1: *mut c_void, arg2: *mut c_void) -> c_int {
    ...
    if rxl.is_null() {
        NETIO_RXQ_UNLOCK!();
        libc::fprintf(c_stderr(), c"netio_rxl_add: unable to create structure.\n".as_ptr());
        return -1;
    }
    ...
    if (netio_get_fd((*rxl).nio) == -1) && libc::pthread_create(addr_of_mut!((*rxl).spec_thread), null_mut(), netio_rxl_spec_thread, rxl.cast::<_>()) != 0 {
        NETIO_RXQ_UNLOCK!();
        libc::fprintf(c_stderr(), c"netio_rxl_add: unable to create specific thread.\n".as_ptr());
        libc::free(rxl.cast::<_>());
        return -1;
    }
...
pub unsafe extern "C" fn netio_rxl_init() -> c_int {
    ...
    if libc::pthread_create(addr_of_mut!(netio_rxl_thread), null_mut(), netio_rxl_gen_thread, null_mut()) != 0 {
        libc::perror(c"netio_rxl_init: pthread_create".as_ptr());
        return -1;
    }
