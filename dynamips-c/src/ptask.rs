...
static mut ptask_thread: libc::pthread_t = unsafe { zeroed() };
...
pub unsafe extern "C" fn ptask_add(cbk: ptask_callback, object: *mut c_void, arg: *mut c_void) -> ptask_id_t {
    ...
    if task.is_null() {
        libc::fprintf(c_stderr(), c"ptask_add: unable to add new task.\n".as_ptr());
        return -1;
    }
...
pub unsafe extern "C" fn ptask_remove(id: ptask_id_t) -> c_int {
    let mut task: *mut *mut ptask_t;
    let p: *mut ptask_t;
    let mut res: c_int = -1;

    PTASK_LOCK!();

    task = addr_of_mut!(ptask_list);
    while !(*task).is_null() {
        if (*(*task)).id == id {
            p = *task;
            *task = (*(*task)).next;
...
#[no_mangle]
pub unsafe extern "C" fn ptask_init(sleep_time: u_int) -> c_int {
    if sleep_time != 0 {
        ptask_sleep_time = sleep_time;
    }

    if libc::pthread_create(addr_of_mut!(ptask_thread), null_mut(), ptask_run, null_mut()) != 0 {
        libc::fprintf(c_stderr(), c"ptask_init: unable to create thread.\n".as_ptr());
        return -1;
    }
