...
//pub const PPC_EXEC_AREA_SIZE: usize = 16; // FIXME cygwin does not have a rust target
...
#[cfg(not(feature = "USE_UNSTABLE"))]
pub const PPC_JIT_IA_HASH_SIZE: usize = 1 << PPC_JIT_IA_HASH_BITS;
...
#[cfg(feature = "USE_UNSTABLE")]
pub const PPC_JIT_VIRT_HASH_SIZE: usize = 1 << PPC_JIT_VIRT_HASH_BITS;
...
pub const PPC_JIT_PHYS_HASH_SIZE: usize = 1 << PPC_JIT_PHYS_HASH_BITS;

macro_rules! PPC_JIT_TARGET_BITMAP_INDEX {
    ($x:expr) => {
        ($x >> 7) & 0x1F
    };
}
macro_rules! PPC_JIT_TARGET_BITMAP_POS {
    ($x:expr) => {
        ($x >> 2) & 0x1F
    };
}
...
pub struct ppc32_jit_tcb {
    ...
    #[cfg(not(feature = "DEBUG_BLOCK_TIMESTAMP"))]
    ...
    #[cfg(feature = "DEBUG_BLOCK_TIMESTAMP")]
...
#[inline(always)]
#[no_mangle]
pub unsafe extern "C" fn ppc32_jit_tcb_get_host_ptr(b: *mut ppc32_jit_tcb_t, vaddr: m_uint32_t) -> *mut u_char {
...
#[inline(always)]
#[no_mangle]
pub unsafe extern "C" fn ppc32_jit_tcb_local_addr(block: *mut ppc32_jit_tcb_t, vaddr: m_uint32_t, jit_addr: *mut *mut u_char) -> c_int {
...
#[inline(always)]
#[no_mangle]
pub unsafe extern "C" fn ppc32_jit_tcb_match(cpu: *mut cpu_ppc_t, block: *mut ppc32_jit_tcb_t) -> c_int {
    let vpage: m_uint32_t = (*cpu).ia & !PPC32_MIN_PAGE_IMASK;
    ((*block).start_ia == vpage) as c_int
}
...
pub unsafe extern "C" fn ppc32_jit_find_by_phys_page(cpu: *mut cpu_ppc_t, phys_page: m_uint32_t) -> *mut ppc32_jit_tcb_t {
    ...
    let mut block: *mut ppc32_jit_tcb_t = *(*cpu).exec_phys_map.add(page_hash as usize);
    while !block.is_null() {
        if (*block).phys_page == phys_page {
            return block;
        }
        block = (*block).phys_next;
    }
...
pub unsafe extern "C" fn ppc32_op_emit_basic_opcode(cpu: *mut cpu_ppc_t, opcode: u_int) {
    let op: *mut jit_op_t = jit_op_get((*cpu).r#gen, 0, opcode);
    ppc32_op_set(cpu, op);
}
...
pub unsafe extern "C" fn ppc32_op_emit_alter_host_reg(cpu: *mut cpu_ppc_t, host_reg: c_int) {
    let op: *mut jit_op_t = jit_op_get((*cpu).r#gen, 0, JIT_OP_ALTER_HOST_REG);
    (*op).param[0] = host_reg;
    ppc32_op_set(cpu, op);
}
...
pub unsafe extern "C" fn ppc32_op_emit_insn_output(cpu: *mut cpu_ppc_t, size_index: u_int, insn_name: *mut c_char) -> *mut jit_op_t {
    let op: *mut jit_op_t = jit_op_get((*cpu).r#gen, size_index as c_int, JIT_OP_INSN_OUTPUT);
    (*op).arg_ptr = null_mut();
...
pub unsafe extern "C" fn ppc32_op_emit_load_gpr(cpu: *mut cpu_ppc_t, host_reg: c_int, ppc_reg: c_int) {
    let op: *mut jit_op_t = jit_op_get((*cpu).r#gen, 0, JIT_OP_LOAD_GPR);
    (*op).param[0] = host_reg;
    (*op).param[1] = ppc_reg;
    (*op).param[2] = host_reg;
    ppc32_op_set(cpu, op);
...
pub unsafe extern "C" fn ppc32_op_emit_store_gpr(cpu: *mut cpu_ppc_t, ppc_reg: c_int, host_reg: c_int) {
    let op: *mut jit_op_t = jit_op_get((*cpu).r#gen, 0, JIT_OP_STORE_GPR);
    (*op).param[0] = host_reg;
    (*op).param[1] = ppc_reg;
    (*op).param[2] = host_reg;
    ppc32_op_set(cpu, op);
...
// EMIT_UPDATE_FLAGS
#[cfg(any(feature = "DYNAMIPS_ARCH_x86", feature = "DYNAMIPS_ARCH_amd64"))]
#[inline]
#[no_mangle]
pub unsafe extern "C" fn ppc32_op_emit_update_flags(cpu: *mut cpu_ppc_t, field: c_int, is_signed: c_int) {
    let op: *mut jit_op_t = jit_op_get((*cpu).r#gen, 0, JIT_OP_UPDATE_FLAGS);
...
pub unsafe extern "C" fn ppc32_op_emit_require_flags(cpu: *mut cpu_ppc_t, field: c_int) {
    let op: *mut jit_op_t = jit_op_get((*cpu).r#gen, 0, JIT_OP_REQUIRE_FLAGS);
    (*op).param[0] = field;
    ppc32_op_set(cpu, op);
}
...
pub unsafe extern "C" fn ppc32_op_emit_set_host_reg_imm32(cpu: *mut cpu_ppc_t, reg: c_int, val: m_uint32_t) {
    let op: *mut jit_op_t = jit_op_get((*cpu).r#gen, 0, JIT_OP_SET_HOST_REG_IMM32);
    (*op).param[0] = reg;
...
extern "C" {
    ...
    #[cfg(any(feature = "DYNAMIPS_ARCH_x86", feature = "DYNAMIPS_ARCH_amd64"))]
    pub fn ppc32_update_cr_set_altered_hreg(cpu: *mut cpu_ppc_t); // TODO replace
}
