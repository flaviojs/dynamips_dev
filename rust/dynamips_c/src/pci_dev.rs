...
// TODO remove
#[no_mangle]
pub extern "C" fn _export_pci_dev(_: *mut pci_device, _: *mut pci_bus, _: *mut pci_bridge, _: *mut pci_io_device) {}
...
unsafe fn pci_bus_show_dev_list(pci_bus: *mut pci_bus) {
    ...
    dev = (*pci_bus).dev_list;
    while !dev.is_null() {
        libc::printf(cstr!("   %-18s: ID %4.4x:%4.4x, Bus %s, Dev. %2d, Func. %2d"), (*dev).name, (*dev).vendor_id, (*dev).product_id, bus_id, (*dev).device, (*dev).function);
