...
static base64digits: &[u8] = c"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".to_bytes();
...
pub unsafe extern "C" fn base64_encode(mut out: *mut c_uchar, mut in_: *const c_uchar, mut inlen: c_int) {
    ...
    if inlen > 0 {
        let mut fragment: c_uchar;

        *out = base64digits[(*in_.add(0) >> 2) as usize];
        out = out.add(1);
        fragment = (*in_.add(0) << 4) & 0x30;
        if inlen > 1 {
...
pub unsafe extern "C" fn base64_decode(mut out: *mut c_uchar, mut in_: *const c_uchar, maxlen: c_int) -> c_int {
    let mut digit1: c_uchar;
    let mut digit2: c_uchar;
    let mut digit3: c_uchar;
    let mut digit4: c_uchar;
    let mut len: c_int = 0;
