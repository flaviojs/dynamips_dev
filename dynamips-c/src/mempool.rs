...
macro_rules! MEMPOOL_LOCK {
    ($mp:expr) => {
        libc::pthread_mutex_lock(std::ptr::addr_of_mut!((*$mp).lock));
    };
}
pub(crate) use MEMPOOL_LOCK;
macro_rules! MEMPOOL_UNLOCK {
    ($mp:expr) => {
        libc::pthread_mutex_unlock(std::ptr::addr_of_mut!((*$mp).lock));
    };
}
pub(crate) use MEMPOOL_UNLOCK;
...
#[inline]
pub unsafe extern "C" fn mp_foreach(pool: *mut mempool_t, cbk: mp_foreach_cbk, arg: *mut c_void) {
...
#[inline]
pub unsafe fn memblock_alloc(size: size_t, zeroed: c_int) -> *mut memblock_t {
...
#[inline]
pub unsafe extern "C" fn memblock_insert(pool: *mut mempool_t, block: *mut memblock_t) {
    MEMPOOL_LOCK!(pool);
...
#[inline]
pub unsafe fn memblock_delete(pool: *mut mempool_t, block: *mut memblock_t) {
    MEMPOOL_LOCK!(pool);
...
#[inline]
pub unsafe fn mp_alloc_inline(pool: *mut mempool_t, size: size_t, zeroed: c_int) -> *mut c_void {
...
pub unsafe extern "C" fn mp_realloc(addr: *mut c_void, new_size: size_t) -> *mut c_void {
    ...
    let total_size: size_t = new_size + size_of::<memblock_t>();
...
pub unsafe extern "C" fn mp_dup(pool: *mut mempool_t, data: *mut c_void, size: size_t) -> *mut c_void {
    let p: *mut c_void = mp_alloc_n0(pool, size);
    if !p.is_null() {
...
pub unsafe extern "C" fn mp_free_all_blocks(pool: *mut mempool_t) {
    let mut block: *mut memblock_t;
    let mut next: *mut memblock_t;
