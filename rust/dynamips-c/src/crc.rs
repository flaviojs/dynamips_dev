...
const CRC32_POLY: c_ulong = 0xedb88320;

/// CRC tables
static mut crc12_array: [m_uint16_t; 256] = [0; 256];
static mut crc16_array: [m_uint16_t; 256] = [0; 256];
static mut crc32_array: [m_uint32_t; 256] = [0; 256];

/// Initialize CRC-12 algorithm
unsafe fn crc12_init() {
    for i in 0..256 as c_int {
...
unsafe fn crc16_init() {
    for i in 0..256 as c_int {
        ...
        crc16_array[i as usize] = crc;
    }
}

/// Initialize CRC-32 algorithm
unsafe fn crc32_init() {
    for n in 0..256 as c_int {
        let mut c: c_ulong = n as c_ulong;
        for _ in 0..8 {
            if (c & 1) != 0 {
                c = CRC32_POLY ^ (c >> 1);
            } else {
        ...
        crc32_array[n as usize] = c as m_uint32_t;
    }
}
