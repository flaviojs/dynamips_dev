...
pub unsafe extern "C" fn vtty_read_and_store(vtty: *mut vtty_t, fd_slot: *mut c_int) {
    ...
    let c: u8 = if let Ok(x) = u8::try_from(c) {
        x
    } else {
        return;
    };
    ...
    match (*vtty).input_state {
        ...
        VTTY_INPUT_TELNET_IYOU => {
            (*vtty).telnet_opt = c.into();
            // if telnet client can support ttype, ask it to send ttype string
            if ((*vtty).telnet_cmd == WILL.into()) && ((*vtty).telnet_opt == TELOPT_TTYPE.into()) {
        ...
        VTTY_INPUT_TELNET_SB2 => {
            (*vtty).telnet_qual = c.into();
            if ((*vtty).telnet_opt == TELOPT_TTYPE.into()) && ((*vtty).telnet_qual == TELQUAL_IS.into()) {
