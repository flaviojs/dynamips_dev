...
macro_rules! ATM_BRIDGE_LOCK {
    ($t:expr) => {
        libc::pthread_mutex_lock(addr_of_mut!((*$t).lock));
    };
}
macro_rules! ATM_BRIDGE_UNLOCK {
    ($t:expr) => {
...
unsafe extern "C" fn atm_bridge_recv_cell(_nio: *mut netio_desc_t, atm_cell: *mut u_char, cell_len: ssize_t, t: *mut c_void, _: *mut c_void) -> c_int {
    ...
    if status == 1 {
        ...
        if ((*t).arc.len > ATM_RFC1483B_HLEN) && 0 == libc::memcmp((*t).arc.buffer.as_mut_ptr().cast::<_>(), atm_rfc1483b_header.as_ptr().cast::<_>(), ATM_RFC1483B_HLEN) {
            netio_send((*t).eth_nio, (*t).arc.buffer.as_mut_ptr().add(ATM_RFC1483B_HLEN).cast::<_>(), (*t).arc.len - ATM_RFC1483B_HLEN);
        }
...
pub unsafe extern "C" fn atm_bridge_create(name: *mut c_char) -> *mut atm_bridge_t {
    ...
    if registry_add((*t).name, OBJ_TYPE_ATM_BRIDGE, t.cast::<_>()) == -1 {
        libc::fprintf(c_stderr(), c"atm_bridge_create: unable to create bridge '%s'\n".as_ptr(), name);
...
pub unsafe extern "C" fn atm_bridge_cfg_setup(t: *mut atm_bridge_t, tokens: *mut *mut c_char, count: c_int) -> c_int {
    ...
    if count != 5 {
        libc::fprintf(c_stderr(), c"ATM Bridge: invalid VPC descriptor.\n".as_ptr());
        return -1;
    }
...
pub unsafe extern "C" fn atm_bridge_handle_cfg_line(t: *mut atm_bridge_t, str_: *mut c_char) -> c_int {
    ...
    let count: c_int = m_strsplit(str_, b':' as c_char, tokens.as_mut_ptr(), ATM_BRIDGE_MAX_TOKENS as c_int);
    if count <= 1 {
        return -1;
    }

    if 0 == libc::strcmp(tokens[0], c"IF".as_ptr()) {
        return atm_bridge_cfg_create_if(t, tokens.as_mut_ptr(), count);
    } else if 0 == libc::strcmp(tokens[0], c"BRIDGE".as_ptr()) {
        return atm_bridge_cfg_setup(t, tokens.as_mut_ptr(), count);
    }

    libc::fprintf(c_stderr(), c"ATM Bridge: Unknown statement \"%s\" (allowed: IF,BRIDGE)\n".as_ptr(), tokens[0]);
    -1
}
...
pub unsafe extern "C" fn atm_bridge_read_cfg_file(t: *mut atm_bridge_t, filename: *mut c_char) -> c_int {
    ...
    while 0 == libc::feof(fd) {
        ...
        ptr = libc::strpbrk(buffer.as_mut_ptr(), c"#\r\n".as_ptr());
        if !ptr.is_null() {
            *ptr = 0;
        }
        ...
        if !libc::strchr(buffer.as_ptr(), b':' as c_int).is_null() {
            atm_bridge_handle_cfg_line(t, buffer.as_mut_ptr());
        }
    }
...
pub unsafe extern "C" fn atm_bridge_start(filename: *mut c_char) -> c_int {
    let t: *mut atm_bridge_t = atm_bridge_create(c"default".as_ptr().cast_mut());
    if t.is_null() {
        libc::fprintf(c_stderr(), c"ATM Bridge: unable to create virtual fabric table.\n".as_ptr());
        return -1;
    }

    if atm_bridge_read_cfg_file(t, filename) == -1 {
        libc::fprintf(c_stderr(), c"ATM Bridge: unable to parse configuration file.\n".as_ptr());
        return -1;
    }

    atm_bridge_release(c"default".as_ptr().cast_mut());
    0
}
