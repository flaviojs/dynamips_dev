...
pub unsafe extern "C" fn netio_filter_find(name: *mut c_char) -> *mut netio_pktfilter_t {
    let mut pf: *mut netio_pktfilter_t;

    pf = pf_list;
    while !pf.is_null() {
        if 0 == libc::strcmp((*pf).name, name) {
            return pf;
        }
        pf = (*pf).next;
    }
...
struct netio_filter_capture {
    pub desc: *mut _sys::pcap_t,
    pub dumper: *mut _sys::pcap_dumper_t,
...
unsafe extern "C" fn pf_capture_free(nio: *mut netio_desc_t, opt: *mut *mut c_void) {
    ...
    if !c.is_null() {
        libc::printf(c"NIO %s: ending packet capture.\n".as_ptr(), (*nio).name);

        // Close dumper
        if !(*c).dumper.is_null() {
            _sys::pcap_dump_close((*c).dumper);
        }

        // Close PCAP descriptor
        if !(*c).desc.is_null() {
            _sys::pcap_close((*c).desc);
        }
...
unsafe extern "C" fn pf_capture_setup(nio: *mut netio_desc_t, opt: *mut *mut c_void, argc: c_int, argv: *mut *mut c_char) -> c_int {
    ...
    if libc::pthread_mutex_init(addr_of_mut!((*c).lock), null_mut()) != 0 {
        libc::fprintf(c_stderr(), c"NIO %s: pthread_mutex_init failure (file %s)\n".as_ptr(), (*nio).name, *argv.add(0));
        libc::free(c.cast::<_>());
        return -1;
    }

    link_type = _sys::pcap_datalink_name_to_val(*argv.add(0));
    if link_type == -1 {
        libc::fprintf(c_stderr(), c"NIO %s: unknown link type %s, assuming Ethernet.\n".as_ptr(), (*nio).name, *argv.add(0));
        link_type = _sys::DLT_EN10MB as _;
    }

    // Open a dead pcap descriptor
    (*c).desc = _sys::pcap_open_dead(link_type, 65535);
    if (*c).desc.is_null() {
        libc::fprintf(c_stderr(), c"NIO %s: pcap_open_dead failure\n".as_ptr(), (*nio).name);
        libc::pthread_mutex_destroy(addr_of_mut!((*c).lock));
        libc::free(c.cast::<_>());
        return -1;
    }
    ...
    (*c).dumper = _sys::pcap_dump_open((*c).desc, *argv.add(1));
    if (*c).dumper.is_null() {
        libc::fprintf(c_stderr(), c"NIO %s: pcap_dump_open failure (file %s)\n".as_ptr(), (*nio).name, *argv.add(0));
        _sys::pcap_close((*c).desc);
        libc::pthread_mutex_destroy(addr_of_mut!((*c).lock));
        libc::free(c.cast::<_>());
        return -1;
    ...
    libc::printf(c"NIO %s: capturing to file '%s'\n".as_ptr(), (*nio).name, *argv.add(1));
    *opt = c.cast::<_>();
    0
}
...
unsafe extern "C" fn pf_capture_pkt_handler(_nio: *mut netio_desc_t, pkt: *mut c_void, len: size_t, opt: *mut c_void) -> c_int {
    let c: *mut netio_filter_capture = opt.cast::<_>();
    let mut pkt_hdr: _sys::pcap_pkthdr = zeroed();

    if !c.is_null() {
        libc::gettimeofday(addr_of_mut!(pkt_hdr.ts).cast::<_>(), null_mut());
        pkt_hdr.caplen = m_min!(len as u_int, _sys::pcap_snapshot((*c).desc) as u_int);
        pkt_hdr.len = len as u_int;

        // thread safe dump
        libc::pthread_mutex_lock(addr_of_mut!((*c).lock));
        _sys::pcap_dump((*c).dumper.cast::<u_char>(), addr_of!(pkt_hdr), pkt.cast::<_>());
        _sys::pcap_dump_flush((*c).dumper);
...
static mut pf_capture_def: netio_pktfilter_t = netio_pktfilter_t {
    name: c"capture".as_ptr().cast_mut(),
...
unsafe extern "C" fn pf_freqdrop_pkt_handler(_nio: *mut netio_desc_t, _pkt: *mut c_void, _len: size_t, opt: *mut c_void) -> c_int {
    ...
    if !data.is_null() {
        match (*data).frequency {
            -1 => return NETIO_FILTER_ACTION_DROP,
            0 => return NETIO_FILTER_ACTION_PASS,
            _ => {
                (*data).current += 1;
...
static mut pf_freqdrop_def: netio_pktfilter_t = netio_pktfilter_t {
    name: c"freq_drop".as_ptr().cast_mut(),
