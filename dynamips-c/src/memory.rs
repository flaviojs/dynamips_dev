...
// 0.5GB value
pub const MTS_SIZE_512M: m_uint32_t = 0x20000000;

// MTS flag bits: D (device), ACC (memory access), C (chain)
pub const MTS_FLAG_BITS: c_int = 4;
pub const MTS_FLAG_MASK: m_uint64_t = 0x0000000f_u64;

// Masks for MTS entries
pub const MTS_CHAIN_MASK: m_uint32_t = 0x00000001;
pub const MTS_ACC_MASK: m_uint32_t = 0x00000006;
pub const MTS_DEV_MASK: m_uint32_t = 0x00000008;
pub const MTS_ADDR_MASK: m_uint64_t = !MTS_FLAG_MASK;

// Device ID mask and shift, device offset mask
pub const MTS_DEVID_MASK: m_uint32_t = 0xfc000000;
pub const MTS_DEVID_SHIFT: c_int = 26;
pub const MTS_DEVOFF_MASK: m_uint32_t = 0x03ffffff;
...
// Memory access flags
pub const MTS_ACC_AE: m_uint32_t = 0x00000002; // Address Error
pub const MTS_ACC_T: m_uint32_t = 0x00000004; // TLB Exception
pub const MTS_ACC_U: m_uint32_t = 0x00000006; // Unexistent
...
#[cfg(not(feature = "USE_UNSTABLE"))]
macro_rules! MTS64_HASH {
    ($vaddr:expr) => {
        ($vaddr >> MTS64_HASH_SHIFT) & MTS64_HASH_MASK
    };
}
#[cfg(not(feature = "USE_UNSTABLE"))]
pub(crate) use MTS64_HASH;

// MTS64 hash on virtual addresses
#[cfg(feature = "USE_UNSTABLE")]
macro_rules! MTS64_SHR {
    ($v:expr, $i:expr) => {
        MTS_SHR!($v,paste::paste!([<MTS64_HASH_SHIFT $i>]))
    };
}
#[cfg(feature = "USE_UNSTABLE")]
macro_rules! MTS64_HASH {
...
#[cfg(not(feature = "USE_UNSTABLE"))]
macro_rules! MTS32_HASH {
    ($vaddr:expr) => {
        ($vaddr >> MTS32_HASH_SHIFT) & MTS32_HASH_MASK
    };
}
#[cfg(not(feature = "USE_UNSTABLE"))]
pub(crate) use MTS32_HASH;

// MTS32 hash on virtual addresses
#[cfg(feature = "USE_UNSTABLE")]
macro_rules! MTS32_SHR {
    ($v:expr,$i:expr) => {
        MTS_SHR!($v,paste::paste!([<MTS32_HASH_SHIFT $i>]))
    };
}
#[cfg(feature = "USE_UNSTABLE")]
macro_rules! MTS32_HASH {
...
pub unsafe extern "C" fn memlog_rec_access(cpu: *mut cpu_gen_t, vaddr: m_uint64_t, data: m_uint64_t, op_size: m_uint32_t, op_type: m_uint32_t) {
    ...
    (*acc).op_size = op_size;
    (*acc).op_type = op_type;
    (*acc).data_valid = (op_type == MTS_WRITE as m_uint32_t) as m_uint32_t;

    (*cpu).memlog_pos = ((*cpu).memlog_pos + 1) & (MEMLOG_COUNT as u_int - 1);
}
...
pub unsafe extern "C" fn memlog_dump(cpu: *mut cpu_gen_t) {
    ...
    for i in 0..MEMLOG_COUNT as u_int {
        ...
        if cpu_get_pc(cpu) != 0 {
            if (*acc).data_valid != 0 {
                libc::snprintf(s_data.as_mut_ptr(), s_data.len(), c"0x%llx".as_ptr(), (*acc).data);
            } else {
                libc::snprintf(s_data.as_mut_ptr(), s_data.len(), c"XXXXXXXX".as_ptr());
            }

            libc::printf(
                c"CPU%u: pc=0x%8.8llx, vaddr=0x%8.8llx, size=%u, type=%s, data=%s\n".as_ptr(),
                (*cpu).id,
                (*acc).iaddr,
                (*acc).vaddr,
                (*acc).op_size,
                if (*acc).op_type == MTS_READ as m_uint32_t { c"read ".as_ptr() } else { c"write".as_ptr() },
                s_data.as_ptr(),
            );
        }
    }
...
pub unsafe extern "C" fn memlog_update_read(cpu: *mut cpu_gen_t, raddr: m_iptr_t) {
    let acc: *mut memlog_access_t = addr_of_mut!((*cpu).memlog_array[(((*cpu).memlog_pos - 1) & (MEMLOG_COUNT as u_int - 1)) as usize]);

    if (*acc).op_type == MTS_READ as u_int {
        match (*acc).op_size {
            1 => {
                (*acc).data = *(raddr as *mut m_uint8_t) as m_uint64_t;
            }
            2 => {
                (*acc).data = vmtoh16(*(raddr as *mut m_uint16_t)) as m_uint64_t;
            }
            4 => {
                (*acc).data = vmtoh32(*(raddr as *mut m_uint32_t)) as m_uint64_t;
            }
            8 => {
                (*acc).data = vmtoh64(*(raddr as *mut m_uint64_t));
            }
            _ => {}
        }
...
unsafe fn physmem_get_hptr(vm: *mut vm_instance_t, paddr: m_uint64_t, op_size: u_int, op_type: u_int, data: *mut m_uint64_t) -> *mut c_void {
    ...
    if ((*dev).flags & VDEVICE_FLAG_SPARSE) != 0 {
        ptr = dev_sparse_get_host_addr(vm, dev, paddr, op_type, addr_of_mut!(cow)) as *mut c_void;
        if ptr.is_null() {
            return null_mut();
        }

        return ptr.add((paddr & VM_PAGE_IMASK as m_uint64_t) as usize);
    }

    if ((*dev).host_addr != 0) && 0 == ((*dev).flags & VDEVICE_FLAG_NO_MTS_MMAP) {
...
pub unsafe extern "C" fn physmem_copy_from_vm(vm: *mut vm_instance_t, mut real_buffer: *mut c_void, mut paddr: m_uint64_t, mut len: size_t) {
    ...
    while len > 0 {
        r = m_min!(VM_PAGE_SIZE as m_uint64_t - (paddr & VM_PAGE_IMASK as m_uint64_t), len as m_uint64_t) as m_uint32_t;
        ...
        real_buffer = real_buffer.add(r as usize);
...
pub unsafe extern "C" fn physmem_dma_transfer(vm: *mut vm_instance_t, mut src: m_uint64_t, mut dst: m_uint64_t, mut len: size_t) {
    ...
    while len > 0 {
        ...
        if sptr.is_null() || dptr.is_null() {
            vm_log(vm, c"DMA".as_ptr().cast_mut(), c"unable to transfer from 0x%llx to 0x%llx\n".as_ptr().cast_mut(), src, dst);
            return;
        }

        sl = VM_PAGE_SIZE - (src & VM_PAGE_IMASK as m_uint64_t) as size_t;
        dl = VM_PAGE_SIZE - (dst & VM_PAGE_IMASK as m_uint64_t) as size_t;
        clen = m_min!(sl, dl);
        clen = m_min!(clen, len);
...
#[no_mangle]
pub unsafe extern "C" fn physmem_strnlen(vm: *mut vm_instance_t, paddr: m_uint64_t, mut maxlen: size_t) -> size_t {
    let mut len: size_t = 0;

    let vm_ram: *mut vdevice = dev_lookup(vm, paddr, TRUE);
    if !vm_ram.is_null() && (*vm_ram).host_addr != 0 {
        let ptr: *mut c_char = ((*vm_ram).host_addr as *mut c_char).add((paddr - (*vm_ram).phys_addr) as usize);
        maxlen = m_min!(maxlen as m_uint64_t, (*vm_ram).phys_len as m_uint64_t - (paddr - (*vm_ram).phys_addr)) as size_t;
        len = libc::strnlen(ptr, maxlen);
    }
...
pub unsafe extern "C" fn physmem_cfind(vm: *mut vm_instance_t, bytes: *mut m_uint8_t, len: size_t, mut first: m_uint64_t, last: m_uint64_t, paddr: *mut m_uint64_t) -> c_int {
    ...
    if buffer.is_null() {
        libc::perror(c"physmem_cfind: malloc".as_ptr());
        return -1;
    }
    i = 0;
    buflen = 0;
    ...
    while !dev.is_null() {
        ...
        if 0 == ((*dev).flags & VDEVICE_FLAG_CACHING) {
            dev = (*dev).next;
        ...
        while first + len as m_uint64_t <= last_dev_addr {
            ...
            if (i == 0 || libc::memcmp(buffer.cast::<_>(), bytes.add(len - i).cast::<_>(), i) == 0) && libc::memcmp(buffer.add(i).cast::<_>(), bytes.cast::<_>(), len - i) == 0 {
                // match found
                if !paddr.is_null() {
                    *paddr = first;
                }
...
pub unsafe extern "C" fn physmem_dump_vm(vm: *mut vm_instance_t, paddr: m_uint64_t, u32_count: m_uint32_t) {
    for i in 0..u32_count {
        vm_log(vm, c"physmem_dump".as_ptr().cast_mut(), c"0x%8.8llx: 0x%8.8x\n".as_ptr().cast_mut(), paddr + (i << 2) as m_uint64_t, physmem_copy_u32_from_vm(vm, paddr + (i << 2) as m_uint64_t));
    }
}
