...
// Internal states
type _DS1620_STATE_ENUM = u_int; // TODO enum
const DS1620_STATE_CMD_IN: _DS1620_STATE_ENUM = 0;
const DS1620_STATE_DATA_IN: _DS1620_STATE_ENUM = 1;
const DS1620_STATE_DATA_OUT: _DS1620_STATE_ENUM = 2;

// Update status register (TH/TL values)
unsafe fn ds1620_update_status(d: *mut ds1620_data) {
    if (*d).temp >= (*d).reg_th as c_int {
        (*d).reg_config |= DS1620_CONFIG_STATUS_THF;
    }

    if (*d).temp <= (*d).reg_tl as c_int {
...
unsafe fn ds1620_cmd_set_state(d: *mut ds1620_data) {
    ...
    match (*d).cmd {
        ...
        DS1620_READ_CONFIG => {
            (*d).state = DS1620_STATE_DATA_OUT;
            (*d).data_len = DS1620_CONFIG_SIZE;
            (*d).data = (*d).reg_config as m_uint16_t;
        }
...
unsafe fn ds1620_exec_cmd(d: *mut ds1620_data) {
    match (*d).cmd {
        ...
        DS1620_WRITE_CONFIG => {
            (*d).reg_config = (*d).data as m_uint8_t;
        }
...
pub unsafe extern "C" fn ds1620_read_data_bit(d: *mut ds1620_data) -> u_int {
    ...
    let val: u_int = (((*d).data >> (*d).data_pos) & 0x1) as u_int;
