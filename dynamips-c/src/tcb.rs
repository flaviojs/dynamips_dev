...
pub const TB_FLAG_SMC: c_int = 0x01; // Self-modifying code
pub const TB_FLAG_RECOMP: c_int = 0x02; // Page being recompiled
pub const TB_FLAG_NOJIT: c_int = 0x04; // Page not supported for JIT
pub const TB_FLAG_VALID: c_int = 0x08;

// Don't use translated code to execute the page
pub const TB_FLAG_NOTRANS: c_int = TB_FLAG_SMC | TB_FLAG_RECOMP | TB_FLAG_NOJIT;
...
// TC descriptor flags
pub const TC_FLAG_REMOVAL: c_int = 0x01; // Descriptor marked for removal
pub const TC_FLAG_VALID: c_int = 0x02;
...
macro_rules! TC_TARGET_BITMAP_INDEX {
    ($x:expr) => {
        ($x >> 7) & 0x1F
    };
}
macro_rules! TC_TARGET_BITMAP_POS {
    ($x:expr) => {
        ($x >> 2) & 0x1F
    };
}
...
pub unsafe extern "C" fn tc_get_host_ptr(tc: *mut cpu_tc_t, vaddr: m_uint64_t) -> *mut u_char {
    let offset: m_uint32_t = ((vaddr & VM_PAGE_IMASK as m_uint64_t) >> 2) as m_uint32_t;
...
pub type _CPU_JIT_ENUM = c_int; // TODO enum
pub const CPU_JIT_DISABLE_CPU: _CPU_JIT_ENUM = 0;
pub const CPU_JIT_ENABLE_CPU: _CPU_JIT_ENUM = 1;
