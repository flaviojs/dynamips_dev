...
unsafe fn set_u32(p: *mut m_uint8_t, i: size_t, v: m_uint32_t) {
    *p.add(i) = (v & 0xFF) as u8;
    *p.add(i + 1) = ((v >> 8) & 0xFF) as u8;
    *p.add(i + 2) = ((v >> 16) & 0xFF) as u8;
    *p.add(i + 3) = ((v >> 24) & 0xFF) as u8;
}
...
unsafe fn boot16(sector: *mut m_uint8_t, fat16: *mut fat16_data) {
    ...
    *sector.add(0xA) = b'S';
    ...
    // Bytes per sector
    set_u16(sector, 0xB, FS_FAT_SECTOR_SIZE as u16);
    ...
    *sector.add(0x26) = 0x29;
    ...
    // Volume ID
    *sector.add(0x27) = (libc::rand() & 0xFF) as u8;
    *sector.add(0x28) = (libc::rand() & 0xFF) as u8;
    *sector.add(0x29) = (libc::rand() & 0xFF) as u8;
    *sector.add(0x2A) = (libc::rand() & 0xFF) as u8;
    ...
    for i in 0..11 {
        ...
        *sector.add(i + 0x2B) = *(*fat16).volume_name.add(i) as u8;
    }
...
unsafe fn write_sector(fd: c_int, lba: m_uint32_t, sector: *mut m_uint8_t) -> c_int {
    ...
    if libc::write(fd, sector.cast::<_>(), FS_FAT_SECTOR_SIZE as usize) != FS_FAT_SECTOR_SIZE as isize {
...
pub unsafe extern "C" fn fs_fat_format16(fd: c_int, begin_lba: m_uint32_t, nr_sectors: m_uint32_t, sects_per_track: m_uint16_t, heads: m_uint16_t, mut volume_name: *const c_char) -> c_int {
    let sector = [0_u8; FS_FAT_SECTOR_SIZE as usize];
    ...
    let total_clusters: u32 = ((*fat16).volume_sectors / (*fat16).sects_per_cluster as u32) + 1;
    (*fat16).fat_sectors = ((total_clusters / (FS_FAT_SECTOR_SIZE / 2)) + 1) as u16;
    ...
    // FAT sectors
    for ifat in 0..(*fat16).nr_fats as u32 {
        let fat_lba: u32 = begin_lba + (*fat16).reserved_sectors as u32 + ifat * (*fat16).fat_sectors as u32;
        ...
        for isec in 1..(*fat16).fat_sectors as u32 {
    ...
    // Root directory
    let rootdir_lba: u32 = begin_lba + (*fat16).reserved_sectors as u32 + ((*fat16).nr_fats as u32 * (*fat16).fat_sectors as u32);
    let rootdir_sectors: u32 = (((*fat16).root_entry_count as u32 * 32) + (FS_FAT_SECTOR_SIZE - 1)) / FS_FAT_SECTOR_SIZE;
    fat16_empty(sector, fat16);
